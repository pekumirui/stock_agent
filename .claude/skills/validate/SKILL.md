---
name: validate
description: データベースの状態とデータ整合性を検証する
---
...
## 検証項目
1. **銘柄マスタ（companies）** - 登録銘柄数、アクティブ銘柄数
2. **株価データ（daily_prices）** - 総レコード数、最新日付、データ欠損チェック
3. **決算データ（financials）** - 総レコード数、最新決算期
4. **整合性** - 外部キー参照の確認、孤立データのチェック

## 検証コマンド
```bash
cd /home/pekumirui/stock_agent && venv/bin/python -c "
...
with get_connection() as conn:
    ...
    ''').fetchone()[0]
    print(f'アクティブ銘柄: {companies}')
    print(f'株価レコード: {prices[0]}, 最新: {prices[1]}')
    print(f'決算レコード: {financials}')
    print(f'孤立データ: {orphans}')
"
```
</validate-skill>
