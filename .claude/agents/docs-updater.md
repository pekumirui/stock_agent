---
name: docs-updater
description: "ソースコード、アーキテクチャ、データベーススキーマ、プロジェクト設定が変更された際に、対応するドキュメント（docs/ディレクトリ、README.md、CLAUDE.mdなど）を更新するエージェント。コード変更、新機能実装、アーキテクチャリファクタリング、スキーママイグレーション、依存関係の更新後に積極的に呼び出すこと。\n\n使用例:\n\n<example>\nContext: 配当データ取得バッチスクリプトを新規作成した場合\nuser: \"scripts/fetch_dividends.py を新規作成して、配当データを取得するバッチを実装して\"\nassistant: \"配当データ取得バッチを実装しました。\"\n<function call to create fetch_dividends.py omitted for brevity>\nassistant: \"新しいスクリプトが追加されたので、docs-updater エージェントを使ってドキュメントを更新します。\"\n<commentary>\n新しいスクリプトがプロジェクトに追加されたため、Task toolでdocs-updaterエージェントを起動し、README.md、CLAUDE.md、および関連するdocs/ファイルを更新する。\n</commentary>\n</example>\n\n<example>\nContext: データベーススキーマに新しいテーブルを追加した場合\nuser: \"新しいテーブル dividend_history を schema.sql に追加して\"\nassistant: \"dividend_history テーブルをスキーマに追加しました。\"\n<function call to modify db/schema.sql omitted for brevity>\nassistant: \"スキーマが変更されたので、docs-updater エージェントを使ってドキュメントを更新します。\"\n<commentary>\nデータベーススキーマが変更されたため、Task toolでdocs-updaterエージェントを起動し、スキーマドキュメント、ER図の参照、データベース構造を説明するドキュメントを更新する。\n</commentary>\n</example>\n\n<example>\nContext: モジュール構成を再編成した場合\nuser: \"lib/ 配下のモジュールを再構成して、parser と client に分けて\"\nassistant: \"モジュールを再構成しました。\"\n<function calls to reorganize files omitted for brevity>\nassistant: \"アーキテクチャが変更されたので、docs-updater エージェントを使ってドキュメントを更新します。\"\n<commentary>\nプロジェクトのアーキテクチャが再編成されたため、Task toolでdocs-updaterエージェントを起動し、ディレクトリ構成ドキュメント、モジュール参照、アーキテクチャドキュメントを更新する。\n</commentary>\n</example>"
model: sonnet
color: green
memory: user
---
