...
- **3エリアレイアウト**: メインテーブル（上）、AIコメント領域（左下）、業績詳細パネル（右下）
- **リアルタイムデータ**: 最新の決算発表を時系列で表示（開示時刻付き）
- **業績詳細パネル**: 行クリックで直近12四半期の累計決算・3か月単独決算・会社予想をYoY%付きで表示
...
**fiscal_end_dateの正確性**: TDnetのXBRLでは、四半期（Q1/Q2/Q3）の`CurrentYearInstant`コンテキストが会計年度末を指す場合があります。`fetch_tdnet.py`は、四半期の場合にタイトルから推定した期末日と検証し、不一致の場合はタイトル推定を優先することで、正確な四半期期末日（Q1=会計年度開始+3ヶ月、Q2=+6ヶ月、Q3=+9ヶ月）を保証します。
